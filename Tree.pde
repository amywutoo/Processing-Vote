...
// I create a "Tree" Class.

class Tree {
  PShape trees;
  float x;    // I create variables to specify the x and y of each Tree.
  float y;
  float z;    // I create "z", a variable I'll use in a formula to modify the Trees position.
  ...
  Tree() {
    ...
    // which it's like to say that the Trees are not moving during the first frame.
    pz = z;
  }

  void update() {
    // In the formula to set the new Trees coordinates
    // I'll divide a value for the "z" value and the outcome will be
    // the new x-coordinate and y-coordinate of the Tree.
    ...
    z = z - 0;
    // when the "z" value equals to 1, I'm sure the Tree have passed the
  ...
  void show() {
    //fill(255);
    //noStroke();
    trees = loadShape("tree.svg");
    // with theese "map", I get the new Tree positions
    ...
    // I use the z value to increase the Tree size between a range from 0 to 16.
    float r = map(z, 45, width/2, 80, 45);
    shape(trees, sx, sy, r, r);

    // Here i use the "pz" valute to get the previous position of the Trees,
